...
suite('Extension Test Suite', () => {
  ...
  test('Should get the correct translation on the status bar', async () => {
    const uri = vscode.Uri.file(path.join(__dirname + testFileLocation));
    ...
    vscode.commands.executeCommand('extension.translate').then((result) => {
      ...
      vscode.commands
        .executeCommand('farsification.copyTranslationText')
        .then(() => {
  ...
  test('Should get the correct translation then insert it.', async () => {
    ...
    await sleep(200);
    vscode.commands
      .executeCommand('extension.translateInsert')
      .then((result) => {
        assert.equal(
          editor.document.getText(editor.selection).indexOf('你好') > -1,
          true
        );
      });
  });
